...
def run_single_experiment(config):
    ...
    # Configure ANN with specific activation function
    num_hidden_layers = len(config['ann_layers']) - 2
    activations = [config['activation_func']] * num_hidden_layers + [None]
    ann = ANN(layer_sizes=config['ann_layers'], activations=activations)
...
if __name__ == "__main__":
    ...
    NUM_RUNS = 10  # Use 10 for final report, 3 for quick tests

    print("--- Experiment 1: Investigating ANN Architecture ---")
    ...
    # We use the static methods directly from the ANN class
    activation_functions = [ANN.relu, ANN.tanh, ANN.logistic]

    for name, layers in architectures.items():
        for func in activation_functions:
            maes = []
            print(f"\n--- Testing Architecture: '{name}' ({layers}) with Activation: '{func.__name__}' ---")
            for i in range(NUM_RUNS):
                ...
                maes.append(mae)
            print(f"Result for '{name}' with '{func.__name__}': Avg MAE = {np.mean(maes):.4f}, Std Dev = {np.std(maes):.4f}")
